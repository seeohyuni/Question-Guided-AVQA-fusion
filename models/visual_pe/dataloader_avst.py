...
def _cached_feature_path(cache, directories, filename):
    path = cache.get(filename)
    if path is None:
        path = _first_existing_feature_path(directories, filename)
        cache[filename] = path
    return path
...
class AVQA_dataset(Dataset):
    def __init__(
        ...
        self.text_feat_dirs = [
            _resolve_text_dir(path)
            for path in _as_path_list(text_feat_dir, text_feat_fallback_dir)
        ]
        ...
        for sample in self.samples:
            ...
            sample["_audio_path"] = _cached_feature_path(
                audio_path_cache,
                self.audio_dirs,
                f"{video_id}.npy",
            )
            sample["_visual_path"] = _cached_feature_path(
                visual_path_cache,
                self.video_res14x14_dirs,
                f"{video_id}.npy",
            )
            sample["_text_path"] = _cached_feature_path(
                text_path_cache,
                self.text_feat_dirs,
                f"{question_id}.npy",
            )
    ...
    def _load_audio_feature(self, audio_path):
    ...
    def _load_visual_feature(self, visual_path):
    ...
    def _load_text_feature(self, text_path):
    ...
    def __getitem__(self, idx):
        ...
        audio = self._load_audio_feature(sample["_audio_path"])
        visual_posi = self._load_visual_feature(sample["_visual_path"])
        text = self._load_text_feature(sample["_text_path"])
